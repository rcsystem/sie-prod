...
use App\Models\ticketsServiceModel;
...
class Tickets extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->requestServiceGralModel = new ticketsServiceModel();
    ...
    public function viewTable()
    {
        ...
        $query = $this->db->query("SELECT ActividadId, Actividad_Actividad FROM cat_ticket_actividad WHERE active_status = 1 AND Actividad_ClasificacionId = 1")->getResult();
        ...
        $query2 = $this->db->query("SELECT TecnicoId, CONCAT(Tecnico_Nombre,' ',Tecnico_ApellidoPaterno) AS nombre FROM cat_ticket_tecnico WHERE Tecnico_AreaId = 1 AND Tecnico_Activo = 1 ORDER BY Tecnico_Nombre ASC")->getResult();
        $query3 = $this->db->query("SELECT id_category, category FROM cat_ticket_category WHERE active_status = 1")->getResult();

        $data = ($company == 1)
            ? ['actvidad' => $query, 'usuarios' => $query1, 'inge' => $query2, 'categorias' => $query3]
            : ['categorias' => $query3];

        return ($this->is_logged) ? view('tickets/tablero', $data) : redirect()->to(site_url());
    }

    public function viewTableServiceGral()
    {
        $company = session()->access_tickets;
        if ($company == 4) {
            ...
            return ($this->is_logged) ? view('tickets/tablero-servicios', $data) : redirect()->to(site_url());
        } else {
            return ($this->is_logged) ? view('tickets/tablero-servicios') : redirect()->to(site_url());
        }
    }
    ...
    public function insertTickets()
    {
        ...
        $costCenter = $this->db->query(
            "SELECT clave_cost_center AS clave 
            FROM cat_cost_center 
            WHERE id_cost_center = 
            (SELECT id_cost_center FROM tbl_users WHERE id_user = $idUser)"
        )->getRow()->clave;
        ...
        if (empty($idInge)) {
            ...
            $query = $this->db->query("SELECT a.id_user, CONCAT(a.`name`,' ',a.surname) AS nombre , a.email,
                ...
                GROUP BY a.id_user
                ORDER BY NO_Tickets ASC LIMIT 1;")->getRow();
            $idInge = $query->id_user;
            $nameInge = $query->nombre;
            $emailInge = $query->email;
            // }
            ...
            if ($idStatus == 3) {
                $dataInsert = [
                    ...
                    'conclud_id' => $idInge,
                    'Ticket_FechaConcluido' => date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . ' + 6 minute')),
                    ...
                    'Ticket_FechaCreacion' => date('Y-m-d H:i:s'),
                    'process_id' => $idInge,
                    'date_process' => date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . ' + 1 minute')),
                    'active_status' => 1,
                ];
            } else if ($idStatus == 2) {
                $dataInsert = [
    ...
    public function insertTicketServiceGral()
    {
        //$idActivity = $this->request->getPost('sel-actividad');
        $description = $this->request->getPost('txt-descripcion');
        ...
        $dataInsert = [
            ...
            'Ticket_EstatusId' => 1,
            'Ticket_Descripcion' => $description,
            'Ticket_UsuarioCreacionId' => session()->id_user,
            'Ticket_UsuarioCreacion' => session()->name . ' ' . session()->surname . ' ' . session()->second_surname,
        ...
        $this->db->transStart();
        $this->requestServiceGralModel->insert($dataInsert);
        $this->notificarServiceEmail($this->db->insertID(), 1);
    ...
    public function ticketsALL()
    {
        ...
        if (session()->access_tickets == 1) {
            $sqlList = (session()->manager_tickets == 1 || session()->id_user == 1063 || session()->id_user == 1 || session()->id_user == 1386)
                ? "SELECT DISTINCT Actividad_AreaId FROM cat_ticket_actividad WHERE active_status = 1 AND Actividad_ClasificacionId = " . session()->access_tickets
                : "SELECT DISTINCT Actividad_AreaId FROM cat_ticket_actividad WHERE active_status = 1 AND ActividadId IN
                    (SELECT ActividadId from tbl_tickets_activity_manager WHERE active_status = 1 AND id_manager = $idUser)";
            ...
            $sqlSearch = ($this->request->getPost('tipo_vista') == 2)
                ? "AND a.Ticket_TecnicoId = $idUser"
                : "AND ( a.id_category IN ($sqlList) OR a.Ticket_TecnicoId = $idUser )";
            /* if (session()->id_user == 1063) {
                echo "antes de validar campos de busqueda" . $sqlSearch . "<br><br><br>";
            } */

            if (session()->manager_tickets == 1 || session()->id_user == 1063 || session()->id_user == 1 || session()->id_user == 1386) {
                ...
                # code...
                if ($sqlPriority != '' || $sqlActivity != '' || $sqlUser != '' || $sqlIng != '') {
                    $sqlSearch = "$sqlIng $sqlPriority $sqlActivity $sqlUser";
                }
            }

            $data = $this->db->query("SELECT DISTINCT a.TicketId, a.Ticket_EstatusId, a.Ticket_Descripcion, a.Ticket_UsuarioCreacion,
                DATE_FORMAT(a.Ticket_FechaCreacion, '%d/%m/%Y   %H:%i') AS Ticket_FechaCreacion,
                concat(b.Tecnico_nombre,' ',b.Tecnico_apellidoPaterno) AS Tecnico_Nombre , c.Actividad_Actividad,
                CASE
                    WHEN a.Ticket_PrioridadId = 1 THEN '#F0F0F0'
                    WHEN a.Ticket_PrioridadId = 2 THEN '#A6F9D9'
                    WHEN a.Ticket_PrioridadId = 3 THEN '#FFE6B2'
                    WHEN a.Ticket_PrioridadId = 4 THEN '#FFBCAC'
                    ELSE '#F0F0F0'
                END AS colorPrioridad,
                CASE
                    WHEN a.Ticket_PrioridadId = 1 THEN '#808668'
                    WHEN a.Ticket_PrioridadId = 2 THEN '#159181'
                    WHEN a.Ticket_PrioridadId = 3 THEN '#BD9007'
                    WHEN a.Ticket_PrioridadId = 4 THEN '#EF4949'
                    ELSE '#F72F00'
                END AS colorPrioridadTXT
            ...
            $query = $this->db->query("SELECT  a.TicketId, a.Ticket_EstatusId, a.Ticket_Descripcion,
                DATE_FORMAT(a.Ticket_FechaCreacion, '%d/%m/%Y   %H:%i') AS Ticket_FechaCreacion, 
                concat(b.Tecnico_nombre,' ',b.Tecnico_apellidoPaterno) AS Tecnico_Nombre , c.Actividad_Actividad,
                CASE
                    WHEN a.Ticket_PrioridadId = 1 THEN '#F0F0F0'
                ...
                FROM tbl_tickets_request AS a JOIN cat_ticket_tecnico AS b ON a.Ticket_TecnicoId = b.TecnicoId
                    LEFT JOIN cat_ticket_actividad AS c ON a.id_activity = c.ActividadId 
                WHERE a.Ticket_UsuarioCreacionId = $idUser 
                AND a.active_status = 1 
                AND Ticket_FechaCreacion > '2023-12-31'
            ORDER BY a.TicketId DESC LIMIT 1000")->getResult();
            $data = ($query) ? $query : 'nuevoTicket';
        }
        // return json_encode('nuevoTicket');
        return ($data) ? json_encode($data) : json_encode(false);
    }

    public function dataTiketsServiceGral()
    {
        $folio = $this->request->getPost('folio');
        $idUser = session()->id_user;
        $company = session()->access_tickets;
        if (session()->access_tickets == 4) {
            $sqlAnd = ($folio != null) ? "AND TicketId LIKE '%$folio%'" : "";
            $data = $this->db->query("SELECT  a.TicketId, a.Ticket_EstatusId , a.Ticket_FechaCreacion, a.Ticket_Descripcion,
                concat(b.Tecnico_nombre,' ',b.Tecnico_apellidoPaterno) AS Tecnico_Nombre , c.Actividad_Actividad 
                FROM tbl_tickets_service_request AS a 
                    LEFT JOIN cat_ticket_tecnico AS b ON a.Ticket_TecnicoId = b.TecnicoId
                    LEFT JOIN cat_ticket_actividad AS c ON a.id_activity = c.ActividadId 
                WHERE a.active_status = 1 $sqlAnd
            ORDER BY a.TicketId DESC LIMIT 1000")->getResult();
        } else {
            $query = $this->db->query("SELECT  a.TicketId, a.Ticket_EstatusId , a.Ticket_FechaCreacion, a.Ticket_Descripcion,
                ...
                FROM tbl_tickets_service_request AS a 
                    LEFT JOIN cat_ticket_tecnico AS b ON a.Ticket_TecnicoId = b.TecnicoId
                    LEFT JOIN cat_ticket_actividad AS c ON a.id_activity = c.ActividadId 
                WHERE a.Ticket_UsuarioCreacionId = $idUser AND a.active_status = 1 
            ORDER BY a.TicketId DESC LIMIT 1000")->getResult();
            $data = ($query) ? $query : 'nuevoTicket';
        }
        // return json_encode('nuevoTicket');
    ...
    public function searchTicketsServiceGral()
    {
        try {
            $company = session()->access_tickets;
            $sqlPriority = ($this->request->getPost('prioridad') == '') ? '' : 'AND a.Ticket_PrioridadId = ' . $this->request->getPost('prioridad');
            ...
            $data = $this->db->query("SELECT  a.TicketId, a.Ticket_EstatusId , a.Ticket_FechaCreacion, a.Ticket_Descripcion,
                concat(b.Tecnico_nombre,' ',b.Tecnico_apellidoPaterno) AS Tecnico_Nombre, c.Actividad_Actividad 
                FROM tbl_tickets_service_request AS a JOIN cat_ticket_tecnico AS b ON a.Ticket_TecnicoId = b.TecnicoId
                LEFT JOIN cat_ticket_actividad AS c ON a.id_activity = c.ActividadId 
                WHERE a.active_status = 1 $sqlPriority $sqlActivity $sqlUser $sqlIng $sqlDate ORDER BY a.TicketId DESC")->getResult();
            return json_encode($data);
        } catch (\Exception $th) {
    ...
    public function dataActivitiesServiceGral()
    {
        $data = $this->db->query("SELECT ActividadId, Actividad_Actividad FROM cat_ticket_actividad WHERE Actividad_AreaId = 4 AND active_status = 1")->getResult();
        return json_encode($data);
    }
    ...
    public function dataToCreateTicket()
    {
        ...
        if (session()->access_tickets != false) {
            if ($idRequest) {
                if (session()->manager_tickets == 1) {
                    $query1 = $this->db->query("SELECT DISTINCT TecnicoId, CONCAT(Tecnico_Nombre,' ',Tecnico_ApellidoPaterno) AS nombre 
                    FROM cat_ticket_tecnico WHERE Tecnico_Activo = 1 AND Tecnico_AreaId = 1")->getResult();
                } else {
                    $query1 = $this->db->query("SELECT DISTINCT TecnicoId, CONCAT(Tecnico_Nombre,' ',Tecnico_ApellidoPaterno) AS nombre 
                        FROM cat_ticket_tecnico WHERE Tecnico_Activo = 1
                        AND TecnicoId IN (
                            SELECT DISTINCT id_manager 
                            FROM tbl_tickets_activity_manager 
                            WHERE active_status = 1 
                            AND ActividadId IN (
                                SELECT id_activity 
                                FROM tbl_tickets_request 
                                WHERE TicketId = $idRequest
                                )
                            )
                    ")->getResult();
                }
            } else {
                $query1 = $this->db->query("SELECT TecnicoId, CONCAT(Tecnico_Nombre,' ',Tecnico_ApellidoPaterno) AS nombre 
                    FROM cat_ticket_tecnico WHERE Tecnico_AreaId = 1 AND Tecnico_Activo = 1")->getResult();
            }
        }
    ...
    public function dataTicket()
    {
        ...
        $query = $this->db->query("SELECT a.Ticket_TecnicoId, a.Ticket_UsuarioCreacionId, a.Ticket_PrioridadId, a.Ticket_Descripcion,
             a.Ticket_Solucion, a.Ticket_UsuarioCreacion, a.Ticket_EstatusId, CONCAT(b.`name`,' ',b.surname) AS nombre, 
             a.Ticket_FechaCreacion, a.Ticket_TecnicoId, a.motive_cancel, a.date_cancel
        ...
        $myticketMgr = ($query->Ticket_TecnicoId == session()->id_user || session()->id_user == 1063) ? true : false;
        $myticketUsr = ($query->Ticket_UsuarioCreacionId == session()->id_user || session()->id_user == 1063) ? true : false;
    ...
    public function dataTicketServiceGral()
    {
        $idRequest = $this->request->getPost('id_requets');
        $query = $this->db->query("SELECT a.Ticket_TecnicoId, a.Ticket_UsuarioCreacionId, a.Ticket_PrioridadId, a.Ticket_Descripcion,
             a.Ticket_Solucion, a.Ticket_UsuarioCreacion, a.Ticket_EstatusId, CONCAT(b.`name`,' ',b.surname) AS nombre, 
             a.Ticket_FechaCreacion, a.Ticket_TecnicoId, a.motive_cancel, a.date_cancel
        FROM tbl_tickets_service_request AS a LEFT JOIN tbl_users AS b ON a.Ticket_TecnicoId = b.id_user WHERE TicketId = $idRequest")->getRow();
        $date = date("d/m/Y", strtotime($query->Ticket_FechaCreacion));
        $hour = date("H:i", strtotime($query->Ticket_FechaCreacion));
        // $query1 = $this->db->query("SELECT * FROM tbl_tickets_accion WHERE Accion_TicketId = $idRequest AND active_status = 1")->getResult();
        $query1 = null;
        $myticketMgr = (($query->Ticket_EstatusId == 1 && session()->access_tickets == 4) || ($query->Ticket_EstatusId > 1 && $query->Ticket_TecnicoId == session()->id_user) /* || session()->id_user == 1063 */) ? true : false;
        $myticketUsr = ($query->Ticket_UsuarioCreacionId == session()->id_user /* || session()->id_user == 1063 */) ? true : false;
    ...
    public function statusTicket()
    {
        try {
            $idRequest = $this->request->getPost('id_request');
            $idStatus = $this->request->getPost('estatus');
            $arrayType = [1 => 'nuevo', 2 => 'date_process', 3 => 'Ticket_FechaConcluido', 4 => 'date_cancel', 5 => 'date_closed'];
            if ($idStatus == 2) {
                $idPriority = $this->request->getPost('opcion');
                $dataUpRequest = [
                    'Ticket_EstatusId' => $idStatus,
                ...
                $solution = $this->request->getPost('opcion');
                $dataUpRequest = [
                    'Ticket_EstatusId' => $idStatus,
                    $arrayType[$idStatus] => date('Y-m-d H:i:s'),
                ...
                $motiveCancel = $this->request->getPost('opcion');
                $dataUpRequest = [
                    'Ticket_EstatusId' => $idStatus,
                    $arrayType[$idStatus] => date('Y-m-d H:i:s'),
                    'motive_cancel' => $motiveCancel,
                ];
            } else if ($idStatus == 5) {
                $qualifyService = $this->request->getPost('opcion');
                $dataUpRequest = [
                    'Ticket_EstatusId' => $idStatus,
                    $arrayType[$idStatus] => date('Y-m-d H:i:s'),
                    'qualify_service' => $qualifyService,
                ];
            }
            $update = $this->requestModel->update($idRequest, $dataUpRequest);
            return ($update) ? json_encode(true) : json_encode(false);
    ...
    public function statusTicketServiceGral()
    {
        try {
            $idRequest = $this->request->getPost('id_request');
            $idStatus = $this->request->getPost('estatus');
            $arrayType = [1 => 'nuevo', 2 => 'date_process', 3 => 'date_conclud', 4 => 'date_cancel', 5 => 'date_closed'];
            $arrayIdType = [1 => 'nuevo', 2 => 'process_id', 3 => 'conclud_id', 4 => 'cancel_id', 5 => 'closed_id'];
            $qualifyService = (!empty($this->request->getPost('opcion'))) ? $this->request->getPost('opcion') : 0;
            if ($idStatus == 2) {
                $idPriority = $this->request->getPost('opcion');
                $dataUpRequest = [
                    'Ticket_TecnicoId' => session()->id_user,
                    $arrayIdType[$idStatus] => session()->id_user,
                    'Ticket_EstatusId' => $idStatus,
                    $arrayType[$idStatus] => date('Y-m-d H:i:s'),
                    'Ticket_PrioridadId' => $idPriority,
                    'qualify_service' => $qualifyService
                ...
                $dataUpRequest = [
                    ...
                    'Ticket_Solucion' => $solution,
                    'qualify_service' => $qualifyService,
                    'cancel_id' => session()->id_user
                    // 'Ticket_FechaModificacion' => date('Y-m-d H:i:s'),
                ];
                $this->notificarServiceEmail($idRequest, 3);
            } else if ($idStatus == 4) {
                $motiveCancel = $this->request->getPost('opcion');
                $dataUpRequest = [
                    $arrayIdType[$idStatus] => session()->id_user,
                ...
                $this->notificarServiceEmail($idRequest, 4);
            } else if ($idStatus == 5) {
                $dataUpRequest = [
                    $arrayIdType[$idStatus] => session()->id_user,
                    'Ticket_EstatusId' => $idStatus,
            ...
            $update = $this->requestServiceGralModel->update($idRequest, $dataUpRequest);
            return ($update) ? json_encode(true) : json_encode(false);
        } catch (\Exception $th) {
            return json_encode('error: ' . $th);
        }
    ...
    public function addChat()
    {
        try {
            ...
            if ($type == 1) {
                ...
                $binder =  '../public/doc/tickets/' . $idRequest . date('Y_m_d_H_i');
    ...
    {
        $query = $this->db->query("SELECT a.Actividad_Actividad AS act ,
            (SELECT ct1.category FROM cat_ticket_category AS ct1 WHERE ct1.id_category = a.Actividad_AreaId) AS class
             FROM cat_ticket_actividad AS a 
             WHERE a.ActividadId = " . $data["id_activity"])
            ->getRow();
        ...
        try {
            ...
            $mail->SMTPAuth = false;
            $mail->Host = 'localhost';
           /*  $mail->Username = 'notificacion@grupowalworth.com';
            $mail->Password = 'N0tific4_2410#'; */
            $mail->Port = 25;

            if ($type == 1) {
                $mail->setFrom('notificacion@walworth.com', 'Nuevo Ticket');
            } else if ($type == 2) {
                $mail->setFrom('notificacion@walworth.com', 'Nueva Acción');
            } else {
                $mail->setFrom('notificacion@walworth.com', 'Cambio de estatus');
            }
    ...
    public function notificarEmailChange($id, $type, $opc)
    {
        ...
        try {
            ...
            if ($type == 2) {
                ...
                $mail->setFrom('notificacion@walworth.com', 'Cancelacion de Ticket');
            } else {
                $mail->setFrom('notificacion@walworth.com', 'Cambio de estatus');
            }
    ...
    public function notificarServiceEmail($id, $type)
    {
        $query = $this->db->query("SELECT a.Ticket_PrioridadId, a.Ticket_UsuarioCreacion, c.email, d.Actividad_Actividad AS act, a.motive_cancel, Ticket_EstatusId AS estatus
            FROM tbl_tickets_service_request AS a LEFT JOIN tbl_users AS b ON a.Ticket_TecnicoId = b.id_user
				JOIN cat_ticket_actividad AS d ON a.id_activity = d.ActividadId
        JOIN tbl_users AS c ON a.Ticket_UsuarioCreacionId = c.id_user WHERE a.TicketId = $id")->getRow();

        /* USAMOS EL HELPER CAMBIAR EMAIL PARA LOS CORREOS DE GRUPOWALWORTH */
        $email = $query->email;
        $title = $query->Ticket_UsuarioCreacion;
        $dir_email = changeEmail($email);
        $mail = new PHPMailer();
        $mail->CharSet = "UTF-8";
        try {
           ...
           /*  $mail->Username = 'requisiciones@walworth.com';
            ...
            if ($type == 1) {
                $mail->setFrom('notificacion@walworth.com', 'Nueva Acción');
                $mail->addAddress('ahuerta@walworth.com.mx', 'ADRIAN ALEJANDRO HUERTA CALDERON');
            ...
            //Content
            $mail->isHTML(true);
            $datas = ['ticket' => $query, 'tipo' => $type, 'id' => $id];
            $email_template = view('notificaciones/notify_tickets_service', $datas);
            $mail->MsgHTML($email_template);
            $mail->Subject =  'Notificación de Tickets Servicios Generales';
            $mail->send();
            return true;
        } catch (Exception $e) {
            return json_encode(false);
    ...
    {
        try {
            if (session()->manager_tickets != false || session()->id_user == 1188 || session()->id_user == 1226 || session()->id_user == 1) {
                $idArea = $this->request->getPost('id_area') ?? session()->manager_tickets;
    ...
    */
    ...
    public function ticketsMaintenanceALL()
    {
        try {
            ...
            WHERE a.active_status = 1 LIMIT 1000")->getResult();
            } else {
                $data = $this->db->query("SELECT a.`status` AS estatus, a.id_order, a.description, a.name_user, a.created_at, b.Actividad_Actividad AS actividad, 
            CONCAT(c.Tecnico_Nombre,' ',c.Tecnico_ApellidoPaterno) AS tecnico, a.cancel_at
            FROM tbl_tickets_maintenance_request AS a
    ...
    public function dataTicketMaintenance()
    {
       ...
       WHERE a.id_order = $idRequest")->getRow();
        ...
        $data =  [
            ...
            'fecha' => $date,
            'hora' => $hour,
            'fecha_cancel' => $dateTimeCancel,
            // 'refaccion' => $query1,
            'nivel' => session()->manager_tickets,
        ];
        return ($query) ? json_encode($data) : json_encode(false);
    }
    ...
    public function notificacionTicketsMaintenance($id, $status)
    {
        try {
           ...
           /*  $mail->Username = 'requisiciones@walworth.com';
            ...
            // Add a recipient
            if ($status == 2) {
                $mail->setFrom('notificacion@walworth.com', 'Ticket Autorizado');
    ...
    public function pdfRequestTickets($id = null)
    {
        ...
        a.motive_cancel,
         a.id_priority, a.name_user, a.name_depto, a.payroll_number, a.job_tecnico, 
            CONCAT(b.Tecnico_Nombre,' ',b.Tecnico_ApellidoPaterno) AS tecnico,
