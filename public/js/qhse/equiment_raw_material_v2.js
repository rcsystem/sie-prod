      ...
      $.ajax({
        ...
        success: function (save) {
          ...
          if (save.hasOwnProperty('xdebug_message')) {
            ...
            /* Swal.fire({ //se le asigna un nombre al swal
              allowOutsideClick: false,
              title: `¡Notificando cambio a HSE!`,
              timerProgressBar: true,
              didOpen: () => {
                Swal.showLoading() //remplaza boton "OK" por animacion de "circulo Cargando"
              },
...
$("#btn_buscar_vale").on('click', function () {
  ...
  $.ajax({
    data: data,
    type: "post",
    url: `${urls}qhse/lista_datos_vales_by_usuario`,
    processData: false,
    contentType: false,
    dataType: "json",
    success: function (resp) {
      if (resp.hasOwnProperty('xdebug_message')) {
        ...
        $("#form_items").append(`<div class="row">
          <div class="col-md-12">
            <H2>VALE #${resp[0].id_request} </2>
            <input type="hidden" name="id_request" value="${resp[0].id_request}">      
          </div>
        </div>`);
