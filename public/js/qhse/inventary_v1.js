...
$(document).ready(function () {
  tbl_inventary = $("#tabla_epp")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
        url: `${urls}qhse/listado_inventario_epp`,
        dataSrc: "",
      },
      lengthChange: true,
      //ordering: true,
    ...
    .DataTable();
  $("#tabla_epp thead").addClass("thead-dark text-center");
});
...
function validaModal() {
    ...
    $("#salida_articulos").on
      ("change", `#cantidad_salida`, function () {
        ...
        let name_product = $("#nombre_articulo").val();
        let data = new FormData();
        data.append("description_product", name_product);
  
        $.ajax({
          data: data, //datos que se envian a traves de ajax
          type: "post", //método de envio
          url: `${urls}qhse/inventario_item`, //archivo que recibe la peticion
  ...
  function handleEdit(id_product) {
    ...
    $.ajax({
      type: "GET",
      url: `${urls}qhse/editar_producto/${id_product}`,
      async: true,
      dataType: "json",
      success: function (resp) {
        //console.log(resp);
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          ...
          url: `${urls}qhse/eliminar_producto`, //archivo que recibe la peticion
          type: "post", //método de envio
          processData: false, // dile a jQuery que no procese los datos
          contentType: false, // dile a jQuery que no establezca contentType
  ...
  function Input(id_product) {
    ...
    let article = $(`#product_${id_product} td`)[1].innerHTML;
  ...
  $("#parametros_epp").submit(function (event) {
    ...
    $.ajax({
      data: data, //datos que se envian a traves de ajax
      url: `${urls}qhse/parametros`, //archivo que recibe la peticion
      type: "post", //método de envio
      processData: false, // dile a jQuery que no procese los datos
      contentType: false, // dile a jQuery que no establezca contentType
      success: function (response) {
  ...
  $("#registrar_articulo").submit(function (event) {
    ...
    $.ajax({
      data: data, //datos que se envian a traves de ajax
      url: `${urls}qhse/entrada`, //archivo que recibe la peticion
      type: "post", //método de envio
      processData: false, // dile a jQuery que no procese los datos
      contentType: false, // dile a jQuery que no establezca contentType
      success: function (response) {
  ...
  function Outlet(id_product) {
    //console.log("Hola Mundo Entrada" + id_product);
    let id_producto = $(`#product_${id_product} td`)[1].innerHTML;
    let article = $(`#product_${id_product} td`)[1].innerHTML;
  ...
  $("#salida_articulos").submit(function (event) {
    ...
    if (error_observacion_salida != "" || error_cantidad_salida != "") {
    ...
    let obs_salidas = ($("#observacion_salida").val().length > 0) ? $("#observacion_salida").val() :'';
  
    data.append("id_producto", $("#id_producto").val());
    data.append("producto", $("#nombre_articulo").val());
    data.append("cantidad", $("#cantidad_salida").val());
    data.append("observacion", obs_salidas);
  
    $.ajax({
      data: data, //datos que se envian a traves de ajax
      url: `${urls}qhse/salidas_epp`, //archivo que recibe la peticion
      type: "post", //método de envio
      processData: false, // dile a jQuery que no procese los datos
      contentType: false, // dile a jQuery que no establezca contentType
      success: function (response) {
