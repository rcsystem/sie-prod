...
$(document).ready(function () {
  tbl_request = $("#tbl_solicitudes_pagadas_talento")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
        url: `${urls}finanzas/tbl_solicitudes_pagadas_talento`,
        dataSrc: "data",
      },
      lengthChange: true,
      ordering: true,
      ...
      columns: [
        {
          ...
          render: function (data, type, row) {
            ...
            return `<input type="checkbox" id="chk-realizada-${row.id_request}" class="chk-realizada" data-id="${row.id_request}" ${checked}>`;
          },
        },
        {
          data: "id_request",
...

        {
          ...
          render: function (data) {
            ...
            action = `onclick="openPdfModal('${urls}${data["ruta_pdf"]}',${data["id_request"]})"`;
            ...
            return `
               <div class="float-left mr-auto">
                 ...
                 <button type="button" class="btn btn-outline-primary btn-naranja btn-sm" onclick="updateIdEpicor(${data["id_request"]} )">
      ...
      createdRow: (row, data) => {
        ...
        let statusCell = $(row).find("td:eq(9)"); // Índice de la columna "Estatus"
        switch (data.request_status) {
          case "1":
    ...
    .DataTable();
  $("#tbl_solicitudes_pagadas_talento thead").addClass("text-center");
});
...
$("#tbl_solicitudes_pagadas_talento").on(
  "change",
  ".chk-realizada",
  function () {
    ...
    $.ajax({
      url: `${urls}finanzas/marcar_realizada_talento`,
      method: "POST",
      data: {
        id_request: id,
        realizada: realizada,
...
$("#actualizarIdEpicor").submit(function (e) {
  ...
  $.ajax({
    url: `${urls}finanzas/actualizar_id_epicor_talento`, // Ruta en CodeIgniter
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
