$(document).ready(function () {
  ...
  tbl_request = $("#tbl_autorizar_adm")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
        url: `${urls}finanzas/tbl_autorizar_solicitudes`,
        dataSrc: "data",
      },
      lengthChange: true,
      ordering: true,
            ...
            let itemsLinks = itemsArray
              .map((item, index) => {
                ...
                if (tipo === "1") {
                  ...
                  action = `onclick="openPdfModal('${urls}public/${ruta}',${data["id_request"]},${data["status_request"]})"`;
                } else if (tipo === "2") {
                  // Excel
...
$("#signBtn").click(function () {
  ...
  $.ajax({
    url: `${urls}finanzas/autorizar_pdf`, // Ruta del controlador en CodeIgniter 4
    type: "POST",
    data: dataFirma,
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        ...
        url: `${urls}finanzas/rechazar_solicitud`, //archivo que recibe la peticion
...
$("#btn-signed").click(function () {
  ...
  $.ajax({
    url: `${urls}finanzas/pago_solicitud_pdf`, // Ruta del controlador en CodeIgniter 4
    type: "POST",
    data: dataFirma,
    contentType: false, // Evita que jQuery establezca un tipo de contenido incorrecto
    processData: false, // Evita que jQuery intente procesar FormData
