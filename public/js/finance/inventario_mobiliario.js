var tbl_activos_mobiliario, tbl_inactivos_mobiliario;
$(document).ready(function () {
  tbl_activos_mobiliario = $("#tbl_finanzas_inventario_mobiliario")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
        url: `${urls}finanzas/todo_inventario_mobiliario`,
        dataSrc: "data",
      },
      lengthChange: true,
      //ordering: true,
      ...
      columns: [
        ...
        { data: "revisado", className: "text-center" },
        {
          ...
          <button type="button" class="btn btn-outline-black btn-sm btn-download" title="Descargar Qr" data-qr="${data["codigo"]}" data-imagen="${data["imagen_qr"]}">
            ...
            <button type="button" class="btn btn-outline-info btn-sm" title="Editar Mobiliario" onclick="editarMobiliario(${data["id_activo"]})">
  ...
  $("#tbl_finanzas_inventario_mobiliario").on("click", ".btn-download", function () {
    var imgUrl = $(this).attr("data-imagen");
    var nombreFactura = $(this).attr("data-qr");

    var link = document.createElement("a");
    link.href = imgUrl;
    link.download = `mobiliario_${nombreFactura}.jpg`;
  ...
  tbl_inactivos_mobiliario = $("#tbl_inventario_inactivo_mobiliario").DataTable({
    processing: true,
    ajax: {
      method: "POST",
      url: `${urls}finanzas/inventario_inactivo_mobiliario`,
      ...
      error: function (xhr, error, thrown) {
        $("#tbl_inventario_inactivo_mobiliario").DataTable().clear().draw(); 
    ...
    order: [[0, "desc"]], // Ordenar por la columna oculta `id_activo`
    createdRow: (row, data) => {
      $(row).attr("id", "mobiliario_inactivo_" + data.id_activo);
    },
  });

  /*Dar de Alta Activos */

  // Evento click para el botón "Guardar mobiliario"
  $("#alta_mobiliario").submit(function (event) {
    ...
    const timerInterval = Swal.fire({
      ...
      title:
        '<i class="far fa-save" style="margin-right: 10px;"></i>¡Guardando Nuevo Mobiliario!',
    ...
    $.ajax({
      url: `${urls}finanzas/alta_activo_mobiliario`, // Reemplaza con la URL de tu servidor
      method: "POST",
      data: formData,
      ...
      success: function (response) {
        ...
        $("#alta_mobiliario")[0].reset();
      },
      error: function (xhr, status, error) {
        console.error("Error en la solicitud AJAX:", error);
        alert("Hubo un error al guardar el mobiliario.");
      },
    });
  });
});
...
function downloadData(id_activo) {
  ...
  $.ajax({
    url: `${urls}finanzas/descargar_carpeta_mobiliario/${id_activo}`,
    method: "GET",
    xhrFields: {
    ...
    success: function (data) {
      ...
      link.download = "mobiliario_" + id_activo + ".zip";
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      Swal.close(timerInterval);
...
function activeChange(item, codigo) {
  Swal.fire({
    icon: "question",
    title: ` <i class="fas fa-power-off" style="margin-right: 10px;"></i>¿Activar el Mobiliario ${codigo}?`,
    confirmButtonText:
    ...
    if (result.isConfirmed) {
      let timerInterval = Swal.fire({
        //se le asigna un nombre al swal
        title: `<i class="far fa-save" style="margin-right: 10px;"></i>¡Mobiliario ${codigo} Activado!`,
        html: "Espere unos Segundos.",
        timerProgressBar: true,
        didOpen: () => {
          Swal.showLoading(); //remplaza boton "OK" por animacion de "circulo Cargando"
      ...
      $.ajax({
        type: "post",
        url: `${urls}finanzas/activar_mobiliario`,
        data: statusContract,
        cache: false,
        dataType: "json",
        contentType: false,
...
function deleteChange(item, codigo) {
  Swal.fire({
    icon: "question",
    title: ` <i class="fas fa-power-off" style="margin-right: 10px;"></i>¿Desactivar el Mobiliario ${codigo}?`,
    confirmButtonText:
    ...
    if (result.isConfirmed) {
      let timerInterval = Swal.fire({
        //se le asigna un nombre al swal
        title: `<i class="far fa-save" style="margin-right: 10px;"></i>¡Mobiliario ${codigo} Desactivado!`,
        html: "Espere unos Segundos.",
        timerProgressBar: true,
        didOpen: () => {
          Swal.showLoading(); //remplaza boton "OK" por animacion de "circulo Cargando"
      ...
      $.ajax({
        type: "post",
        url: `${urls}finanzas/desactivar_mobiliario`,
        data: statusContract,
        cache: false,
        dataType: "json",
        contentType: false,
        ...
        success: function (save) {
          ...
          if (save.hasOwnProperty("xdebug_message")) {
            ...
            Swal.fire({
              ...
              text: `Se ha desactivado el mobiliario ${codigo} correctamente`,
            });
          } else {
            Swal.fire({
              icon: "error",
...
function abrirMobiliarioModal() {
  var miModal = new bootstrap.Modal(document.getElementById("mobiliarioModal"));
  miModal.show();
}

function resetTablas() {
  tbl_activos_mobiliario.ajax.reload(null, false);
  tbl_inactivos_mobiliario.ajax.reload(null, false);
}

// Evento click para el botón "Editar"
function editarMobiliario(id_activo) {
  // Obtener el ID del activo
  const idActivo = id_activo;

  // Realizar una solicitud AJAX para obtener los datos del activo
  $.ajax({
    url: `${urls}finanzas/editar_mobiliario`, // Ruta para obtener los datos
    method: "post",
    ...
    success: function (response) {
      if (response.status === "success") {
        ...
        // Abrir el modal
        $("#editarMobiliarioModal").modal("show");
      } else {
...
$("#form_editar_mobiliario").submit(function (event) {
  ...
  const timerInterval = Swal.fire({
    //se le asigna un nombre al swal
    title:
      '<i class="far fa-save" style="margin-right: 10px;"></i>¡Guardando Mobiliario!',
    html: "Espere unos Segundos.",
    timerProgressBar: true,
    didOpen: () => {
      Swal.showLoading(); //remplaza boton "OK" por animacion de "circulo Cargando"
  ...
  // Enviar los datos por AJAX
  $.ajax({
    url: `${urls}finanzas/actualizar_mobiliario`, // Reemplaza con la URL de tu servidor
    method: "POST",
    data: formData,
    processData: false, // Evitar que jQuery procese los datos
    contentType: false, // Evitar que jQuery establezca el tipo de contenido
    success: function (response) {
      ...
      Swal.fire({
        icon: "success",
        title: "¡Mobiliario guardado correctamente.!",
        text: "",
      });
      // Limpiar el formulario después de guardar
      //$("#form_editar_activos")[0].reset();
      resetTablas();
      $("#editarMobiliarioModal").modal("toggle");
    },
    error: function (xhr, status, error) {
      console.error("Error en la solicitud AJAX:", error);
