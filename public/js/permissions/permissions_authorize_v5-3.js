...
$(document).ready(function () {
  tbl_permissions = $("#tabla_autorizar_permisos").dataTable({
    processing: true,
    ajax: {
      method: "post",
      url: `${urls}permisos/autorizar-permisos`,
      dataSrc: "",
    },
    lengthChange: true,
    ordering: true,
    ...
    columnDefs: [
      {
        targets: 8,
        render: function (data, type, full, meta) {
          change = (data["num_permiso_mes"] > 3 || data["estatus"] != "Pendiente") ? '' : `onClick=handleChange(${data["id_es"]})`;
          btn = (data["num_permiso_mes"] > 3 || data["estatus"] != "Pendiente") ? 'secondary' : 'primary';
  ...
  tbl_vacations = $("#tabla_autorizar_vacaciones").dataTable({
    processing: true,
    ajax: {
      method: "post",
      url: `${urls}permisos/autorizar-vacaciones`,
      dataSrc: "",
    },
    lengthChange: true,
    ordering: true,
      ...
      },
      {
        ...
        render: function (data, type, full, meta) {
          ...
          var mes = (objFechaSalida.getMonth() + 1)
            ...
            .padStart(2, "0");
          if (dia == 32) {
            dia = "01"; var mes = (objFechaSalida.getMonth() + 2)
              .toString()
              .padStart(2, "0");
          }
    ...
    columnDefs: [
      {
        targets: 7,
        render: function (data, type, full, meta) {
          change = (data["estatus"] != "Pendiente") ? '' : `onClick=handleChangeVacation(${data["id_vcns"]})`;
          btn = (data["estatus"] != "Pendiente") ? 'secondary' : 'primary';
          return `<div class=" mr-auto">
              <button type="button" class="btn btn-outline-${btn} btn-sm " title="Autorizar Vacaciones" ${change}>
                <i class="fas fa-user-check"></i>
              </button>
  ...
  $.ajax({
    ...
    success: function (resp) {
      if (resp != false) {
        ...
        action = (id_folio > 8694) ? `onclick="verFechas(${id_folio})"` : 'disabled'
