...
$(document).ready(function () {
  tbl_equipos_asignados = $("#tbl_equipos_asignados")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return ` <div class="mr-auto">
                          <!-- <button type="button" class="btn btn-primary btn-sm" title="Responsiba equipo" onclick="Responsiba(${
                            data["ID"]
                          })">
                              <i class="far fa-edit"></i>
                          </button> -->
                          <a href="${urls}sistemas/pdf-responsiva-asignacion/${$.md5(
              key + data["folio"]
            )}" target="_blank" title="Responsiba equipo" class="btn btn-danger btn-sm">
...
$("#procesos").submit(function (e) {
  ...
  if (tipo.length == 0) {
    ...
    if (tipo == 1) {
      ...
      if (arrayItemsEquip.length > 0) {
        arrayItemsEquip.forEach((item) => {
          const otro_equipos = document.getElementById("otros_equipos_" + item)
            .value.length;
          const div_otro_equipos = document.getElementById(
            "div_otros_equipos_" + item
          );
          if (otro_equipos == 0) {
      ...
      if (arrayItemsInv.length > 0) {
        arrayItemsInv.forEach((item) => {
          const inventario = document.getElementById("inventario_" + item).value
            .length;
          const div_inventario = document.getElementById(
            "div_inventario_" + item
          );
          if (inventario == 0) {
    ...
    if (tipo == 2) {
      ...
      if (
        $("#id_request_ant").val().length > 0 &&
        $("#id_equipo2").val().length > 0
      ) {
        ...
        if (tipo != 3) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              window.open(
                `${urls}sistemas/pdf-responsiva-asignacion/${$.md5(
                  key + save
                )}`,
                "_blank"
              );
...
function proceso(params) {
  ...
  if (params == 4) {
    $("#div_formulario")
      .append(`<h4><i class="fas fa-file-upload" style="margin-right: 10px;"></i>Datos Renovacion</h4>
    ...
    $("#div_formulario")
      .append(`<hr><h4><i class="fas fa-file-upload" style="margin-right: 10px;"></i>Datos Recolección</h4>
            <div class="row">
      ...
      success: function (respEquip) {
        if (respEquip) {
          respEquip.usuarios.forEach((users) => {
            $("#id_user").append(
              `<option value="${users.id_user}">${users.user_name}</option>`
            );
          });
        }
      },
      ...
      if (resp.Equip) {
        if (item != 1) {
          if (resp.Equip.status_equip > 1) {
            var error =
              resp.Equip.status_equip == 2
                ? "Equipo ya esta Asignado"
                : "Equipo incorrecto";
          ...
          if (resp.Equip.status_equip == 1) {
            var error =
              resp.Equip.status_equip == 1
                ? "Equipo esta Almacenado"
                : "Equipo incorrecto";
            $("#error_" + campo_opcion).text(error);
            $("#" + campo_opcion).addClass("has-error");
            $("#" + campo_otro).removeClass("has-error");
          ...
          if (tipo_proceso == 2) {
            $("#div_nombre_user").append(
              `<h4>${resp.Request.user_name}</h4> <input type="hidden" name="id_user" value="${resp.Request.id_user}">`
            );
...
function getDataUserRecolet(thiss) {
  ...
  $.ajax({
    ...
    success: function (resp) {
      if (
        resp.equipos.length > 0 ||
        resp.suministros.length > 0 /* || resp.prestamos */
      ) {
        i = 1;
        ...
        resp.equipos.forEach((item) => {
          ...
          $("#div_items_retirar")
            .append(`<div class="row" style="padding-bottom:10px;padding-top:10px;${bg_color}">
                        <div class="col-md-1">
...
function addItems(tipo) {
  if (tipo == 1) {
    ...
    $("#div_items_asignacion")
      .append(`<div class="row" style="margin-bottom: 10px;" id="div_inv_clon_${contInv}">
            <div class="col-md-9">
    ...
    $.ajax({
      ...
      success: function (listResponse) {
        if (listResponse) {
          $(`#inventario_${contInv}`).append(
            `<option value="">Inventario...</optino>`
          );
          listResponse.forEach((element) => {
            $(`#inventario_${contInv}`).append(
              `<option value="${element.id_product}">${element.product}</option>`
            );
          });
        } else {
          $(`#inventario_${contInv}`).append(
            `<option value="">Sin Equipos Disponible</optino>`
          );
        }
      },
    });
    ...
    $("#div_items_asignacion")
      .append(`<div class="row" style="margin-bottom: 10px;" id="div_equip_clon_${contEquip}">
            <div class="col-md-9">
    ...
    $.ajax({
      ...
      success: function (listResponse) {
        if (listResponse) {
          $(`#otros_equipos_${contEquip}`).append(
            `<option value="">Equipos...</option>`
          );
          listResponse.forEach((element) => {
            $(`#otros_equipos_${contEquip}`).append(
              `<option value="${element.id_equip}">${element.equipo}</option>`
            );
          });
        } else {
          $(`#otros_equipos_${contEquip}`).append(
