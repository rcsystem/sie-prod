...
$("#permiso_medico").submit(function (e) {
  e.preventDefault();
  console.log("inasisten: ",inasistencia);
...
$("#sistemas").on("change", function () {
  ...
  if ($("#sistemas").val() == "OTRO" || $("#sistemas").val() == "otro") {
    $("#div_otro_sistema").append(`
        ...
        <input type="text" name="otro_sistema" id="otro_sistema" class="form-control" onchange="limpiarError(this)">
        ...
        $.ajax({
          ...
          success: function (turns) {
            if (turns != false) {
              turns.forEach((key) => {
                $("#turno").append(
                  `<option value="${key.id}">${key.name_turn}</option>`
                );
              });
            } else {
              Swal.fire({
...
$("#tipo_permiso").on("change", function (e) {
  ...
  $("#div_inasistencia").empty();
  $("#div_salida").empty();
  if ($("#tipo_permiso").val() == 1 || $("#tipo_permiso").val() == 6) {
    $("#div_entrada").empty();
    salida = true;
    $("#div_salida").append(`<div class="form-group col-md-6">
            <label for="fecha_salida">Fecha de Salida:</label>
            <input type="date" name="fecha_salida" id="fecha_salida" class="form-control" onchange="limpiarError(this)">
            <div class="text-danger" id="error_fecha_salida"></div>
        </div>
        <div class="form-group col-md-6">
            <label for="hora_salida">Hora de Salida:</label>
            <input type="time" name="hora_salida" id="hora_salida" class="form-control" onchange="limpiarError(this)">
  ...
  if (
    ...
    $("#div_inasistencia").append(`<div class="form-group col-md-6">
            <label for="fecha_inicio">Dias de Inasistencia</label>
            <input type="date" name="fecha_inicio" id="fecha_inicio" style="background-color: white" class="form-control" onchange="limpiarError(this)">
  ...
  if ($("#tipo_permiso").val() == 2) {
    ...
    // Suponiendo que tienes un <select> con el id 'miSelect'
    var nuevoOption = new Option(
      "Atención psicológica",
      "Atención psicológica"
    );
    $("#motivo").append(nuevoOption);
  } else {
    // Elimina la opción si el tipo es diferente de 2
    $("#motivo option[value='Atención psicológica']").remove();
  }

  /** RAFAEL CRUZ CAMBIO PERMISO DE PSICOLOGIA **/
  ...
  if ($("#tipo_permiso").val() == 2) {
    ...
    $("#div_entrada").append(`<div class="form-group col-md-6">
            <label for="fecha_entrada">Fecha de Entrada:</label>
            <input type="date" name="fecha_entrada" id="fecha_entrada" class="form-control" onchange="limpiarError(this)">
            <div class="text-danger" id="error_fecha_entrada"></div>
        </div>
        <div class="form-group col-md-6">
            <label for="hora_entrada">Hora de Entrada:</label>
            <input type="time" name="hora_entrada" id="hora_entrada" class="form-control" onchange="limpiarError(this)">
            <div class="text-danger" id="error_hora_entrada"></div>
        </div>`);
  }
});
