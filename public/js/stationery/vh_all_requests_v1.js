...
$(document).ready(function () {
  tbl_inventary = $("#tabla_solicitudes")
    .dataTable({
      ...
      ajax: {
        method: "post",
        url: `${urls}papeleria/todas_solicitudes_vh`,
        dataSrc: "",
      },
      lengthChange: true,
      ordering: true,
      ...
      columns: [
        ...
        },
        {
          data: null,
          render: function (data, type, full, meta) {
            switch (parseInt(data["request_status"])) {
              case 1:
                return `<span class="badge bg-secondary">Enviado</span>`;
                break;

              case 2:
                return `<span class="badge bg-success">Concluido</span>`;
                break;
              case 3:
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            ...
            let accion = (data["request_status"]== 1)
            ? `<button type="button" class="btn btn-primary btn-sm" title="Editar Suministro"  onClick=Edit(${data["id_request"]})>
                <i class="far fa-edit"></i>
            </button>`
            :`<button type="button" class="btn btn-secondary btn-sm" title="Editar Suministro">
                <i class="far fa-edit"></i>
            </button>`;
