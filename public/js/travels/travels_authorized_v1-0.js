...
$(document).ready(function () {
  tbl_viaticos = $("#tabla_autorizar_viaticos")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
      ...
      columns: [
...

        {
          ...
          render: function (data, type, full, meta) {
            const color =
              data["request_status"] == 1 ? "outline-primary" : "secondary";
            const onclick =
              data["request_status"] == 1
                ? `onClick="handleAutorizeTravel(${data["folio"]})"`
                : "";
            // Director-> 252
            btnAuthorize =
              data["type_travel"] == 1 && data["access_direct"] != "true"
                ? ``
                : ``;
            const btnAuthorizeVictor =
              data["access_direct"] == "true"
            ...
            <!-- ${btnAuthorizeVictor} -->
              <a href="javascript:void(0);" class="btn btn-outline-danger btn-sm" data-toggle="modal" data-target="#pdfModal" data-url="${urls}viajes/ver-permisos/${$.md5(
              key + data["folio"]
            )}">
  ...
  tbl_gastos = $("#tabla_autorizar_gastos")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
      ...
      columns: [
...

        {
          ...
          render: function (data, type, full, meta) {
            ...
            const onclick =
              data["request_status"] == 1
                ? `onClick="onClick=handleAutorizeExpenses(${data["folio"]})"`
                : "";
...
function handleAutorizeExpenses(id_folio) {
  Swal.fire({
    title:
      '<i class="fas fa-file-signature" style="margin-right: 10px;"></i>¿Autorizar Solicitud de Gasto?',
    showDenyButton: true,
    showCancelButton: true,
    confirmButtonText:
...
function handleAutorizeTravel(id_folio) {
  Swal.fire({
    title:
      '<i class="fas fa-file-signature" style="margin-right: 10px;"></i>¿Autorizar Solicitud de Viatico?',
    showDenyButton: true,
    showCancelButton: true,
    confirmButtonText:
      '<i class="fas fa-check" style="margin-right: 10px;"></i>Confirmar',
...
function actualizarRequest(status, id, type) {
  let timerInterval = Swal.fire({
    //se le asigna un nombre al swal
    title:
      '<i class="fas fa-envelope-open-text" style="margin-right: 10px;"></i>¡Notificando a Recursos Humanos!',
    html: "Espere unos Segundos.",
    timerProgressBar: true,
    didOpen: () => {
      Swal.showLoading(); //remplaza boton "OK" por animacion de "circulo Cargando"
