...
$(document).ready(function () {
  ...
  tbl_provee_estadias = $("#tabla_proveedores_estadias")
  .dataTable({
    processing: true,
    ajax: {
      method: "post",
      url: `${urls}qhse/proveedores_visitas_estadias`,
      dataSrc: "",
    },
    lengthChange: true,
    ordering: true,
...
$("#tabla_proveedores_estadias thead").addClass("thead-dark text-center");



  tbl_permissions = $("#tabla_permisos").dataTable({
      processing: true,
      ajax: {
        method: "post",
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            ...
            hora_salida= '';
            if (data["in_data"] != "-----") {
              hora_entrada = (data["confirm_hora_entrada"] == null && data["estatus"] == 'Autorizada')
                ? `<button type="button" class="btn btn-outline-success btn-sm " title="Hora de Entrada"  onClick=handleEnter(${data["id_es"]},1)>
                    ENTRADA
                    </button>`
                : `<button type="button" class="btn btn-secondary btn-sm " title="Hora de Entrada">
                    ENTRADA
                  </button>` ;
            }
            if (data["out_data"] != "-----") {
              hora_salida = (data["confirm_hora_salida"] == null && data["estatus"] == 'Autorizada')
                ? `<button type="button" class="btn btn-outline-primary btn-sm " title="Hora de Salida"  onClick=handleEnter(${data["id_es"]},2)>
                    SALIDA
                  </button>`
                : `<button type="button" class="btn btn-secondary btn-sm " title="Hora de Salida">
                    SALIDA
                  </button>` ;
            }
            return ` 
            <div class="pull-right mr-auto"> <a href="${urls}permisos/ver-permisos/${$.md5(key + data["id_es"])}" target="_blank" class="btn btn-outline-info btn-sm">
            <i class="fas fa-eye"></i>
            </a>
            ${hora_salida}
            ${hora_entrada}
  ...
  tbl_provee = $("#tabla_proveedores_visitas").dataTable({
      processing: true,
      ajax: {
        method: "post",
  ...
  tbl_over_time = $("#tabla_tiempo_extra").dataTable({
      processing: true,
      ajax: {
        method: "post",
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            return ` <div class="pull-right mr-auto">
                      <a href="${urls}qhse/ver-tiempo-extra/${$.md5(key + data["id"])}" target="_blank" class="btn btn-info btn-sm">
                            <i class="fas fa-eye"></i>
                      </a>
                    </div> `;
          },
...
function handleEnter(id_folio, tipo) {
  showTime();
