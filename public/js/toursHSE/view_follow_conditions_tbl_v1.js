...
$(document).ready(function () {
  tbl_condiciones_inseguras = $("#tbl_incidencias")
    .dataTable({
      processing: true,
      ajax: {
        method: "post",
        url: `${urls}recorridos-HSE/todas_incidencias_condiciones`,
        dataSrc: "",
      },
      lengthChange: true,
      ordering: true,
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            if (data["url_image"] != null) {
              return `<img src="${data["url_image"]}" width="100" height="70" style="cursor: pointer;" onclick="showImageInModal('${data["url_image"]}')">`;
            } else {
              return `<span class="badge badge-info">Sin imagen</span>`;
            }
          },
          title: "ESTADO",
          className: "text-center",
...
function mostrarImagenPrevia(campo) {
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function (e) {
      ...
      $("#btn_" + input.id).append(
        `<i class="fas fa-camera"  style="margin-right: 10px;"></i>RETOMAR FOTO`
      );
    };
